...
export class SqliteQueueStore implements QueueStore {
  ...
  async setQueue(itemIds: string[], startIndex: number = 0): Promise<void> {
    ...
    const tx = this.conn.transaction((ids: string[]) => {
      clearStmt.run()
      ids.forEach((id, idx) => insertStmt.run({ media_item_id: id, position: idx }))
  ...
  async remove(itemId: string): Promise<void> {
    ...
    const deleteStmt = this.conn.prepare(`DELETE FROM playback_queue WHERE media_item_id = ?`)
    const shiftStmt = this.conn.prepare(`
      UPDATE playback_queue
      SET position = position - 1
      WHERE position > @pos
  ...
  async getQueue(): Promise<string[]> {
    const rows = this.conn
      .prepare(`SELECT media_item_id FROM playback_queue ORDER BY position ASC`)
      .all()
