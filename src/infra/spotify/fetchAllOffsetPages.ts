...
export const fetchAllOffsetPages = async <T, A extends any[] = any[]>(
  ...
  while (true) {
    ...
    try {
      ...
      logger.error("[Pagination] Error fetching page", err)
      return undefined
    }
    ...
    if (!page || !Array.isArray(page.items) || page.items.length === 0) {
      if (iteration == 0) {
        logger.warn(`Finished fetchAllOffsetPages after ${iteration} pages`)
      } else {
        logger.debug(`Finished fetchAllOffsetPages after ${iteration} pages`)
      }
      return results
    }
