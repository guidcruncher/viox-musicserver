...
import type { AudioSourceItemType, MediaItem, MediaSourceRef, TrackStore } from "@/types"
...
export class SqliteTrackStore implements TrackStore {
  ...
  async upsert(items: MediaItem[]): Promise<void> {
    try {
      ...
      INSERT INTO tracks (
        ...
        title, subtitle, artist, album, image_url, duration_ms, is_live, mbid, isrc, track
      ) VALUES (
        @id, @source, @item_type, @source_id, @parent_source_id, @source_uri,
        @title, @subtitle, @artist, @album, @image_url, @duration_ms, @is_live, @mbid, @isrc, @track
      )
  ...
  async remove(id: string): Promise<void> {
    this.conn.prepare(`DELETE FROM tracks WHERE id = ?`).run(id)
  }

  async get(id: string): Promise<MediaItem | undefined> {
    const row = this.conn.prepare(`SELECT * FROM tracks WHERE id = ?`).get(id)
  ...
  async list(): Promise<MediaItem[]> {
    const rows = this.conn.prepare(`SELECT * FROM tracks ORDER BY title ASC`).all()
  ...
  async listByParentSourceId(parentSourceId: string): Promise<MediaItem[]> {
    const rows = this.conn
      .prepare(
        `
      SELECT a.*, CASE WHEN b.id IS NULL THEN 0 ELSE 1 END AS library
  ...
  async listByParentSourceIdWithPaging(
    ...
    `,
      ...
      .all(parentSourceId, limit, offset)
    return rows.map((r: any) => this.fromRow(r))
  }
