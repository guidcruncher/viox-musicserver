...
import type { AudioSourceItemType, MediaItem, MediaSourceRef } from "@/types"
import { CacheItemStore } from "@/types"
import { toArray } from "@/utils"
import { normalizeUrl } from "@/utils/normalizers"

export class SqliteCacheStore implements CacheItemStore {
...
ON CONFLICT(id) DO UPDATE SET
  ...
  async cleanup(): Promise<void> {
    try {
      this.conn
        ...
        WHERE id IN (SELECT id from media_items) 
        OR id IN (SELECT id from radio_items) 
        OR id IN (SELECT id from subscriptions) 
        OR id IN (SELECT id from subscriptionepisodes)
        OR 
  ...
  async remove(id: string): Promise<void> {
    this.conn.prepare(`DELETE FROM media_cache WHERE id = ?`).run(id)
  }
  ...
  async listByItemTypes(
    ...
    const rows = this.conn
      .prepare(
        `
      SELECT a.*
      , CASE WHEN b.id IS NULL THEN 0 ELSE 1 END AS library
         FROM media_cache AS a
        LEFT JOIN media_items AS b ON a.id = b.id
