...
import type { LibraryStore, MediaItem, MediaSourceRef, PlaylistStore, TrackStore } from "@/types"
...
export class SpotifyImportService {
  ...
  async importUserTracks(): Promise<void> {
    ...
    if (!res) {
      logger.warn("[SpotifyImport] No saved tracks returned from Spotify")
      return
    }
    ...
    logger.debug(`Imported ${items.length} tracks`)
  }
  ...
  async importUserAlbums(): Promise<void> {
    ...
    if (!res) {
      logger.warn("[SpotifyImport] No saved albums returned from Spotify")
      return
    }
    ...
    for (const entry of res) {
      ...
      if (normalized) {
        const albumTracks: any = await fetchAllOffsetPages((offset) =>
          this.client.getAlbumTracks(normalized.sourceRef.sourceId, offset, 50),
        )
        if (albumTracks) {
          const normalizedTracks = albumTracks.map((i: any) => {
            const mapped = this.normalize.normalize(i)
            mapped.sourceRef.parentSourceId = normalized.sourceRef.sourceId
            return mapped
          })
          this.tracks.upsert(normalizedTracks)
        }
    ...
    await this.library.upsert(items)
    logger.debug(`Imported ${items.length} albums`)
  }
  ...
  async importUserShows(): Promise<void> {
    ...
    if (!res) {
      logger.warn("[SpotifyImport] No saved shows returned from Spotify")
      return
    }
    ...
    await this.library.upsert(items)
    logger.debug(`Imported ${items.length} shows`)
  }
  ...
  async importUserEpiaodes(): Promise<void> {
    ...
    if (!res) {
      logger.warn("[SpotifyImport] No saved episodes returned from Spotify")
      return
    }
    ...
    await this.library.upsert(items)
    logger.debug(`Imported ${items.length} episodes`)
  }

  async importFollowedArtists(): Promise<void> {
    const res: any = await this.client.getMyFollows("artist")
    //fetchAllOffsetPages((offset) => this.client.getMyFollowedArtists(offset))

    if (!res) {
      logger.warn("[SpotifyImport] No followed artists returned from Spotify")
      return
    }
    ...
    await this.library.upsert(items)
    logger.debug(`Imported ${items.length} followed artists`)
  }
  ...
  async importUserPlaylists(): Promise<void> {
    ...
    if (!res) {
      logger.warn("[SpotifyImport] No playlists returned from Spotify")
      return
    }
  ...
  async importPlaylist(playlistId: string): Promise<void> {
    ...
    if (!playlist) {
      logger.warn(`[SpotifyImport] Playlist ${playlistId} returned no data`)
      return
    }
    ...
    if (!rawTracks) {
      logger.warn(`[SpotifyImport] No tracks returned for playlist ${playlistId}`)
      return
    }
    ...
    logger.info(`[SpotifyImport] Imported playlist '${name}' (${items.length} items)`)
  }
  ...
  private async findLocalPlaylistId(ref: MediaSourceRef): Promise<string | null> {
    const all = await this.playlists.listPlaylist()
